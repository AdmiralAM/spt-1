...
- Preserves the historical 31-quest / 11-offer 0.1.0 identity set while adding the accepted Operations, relationship progression and bounded storefront expansion.
- Uses two staged Arsenal tracks instead of exposing every weapon family at once.
- Keeps full quest requirements and rewards in readable multiline EN/RU descriptions while using concise objective rows.
- Provides native explicit accept/complete lifecycle, success mail, rewards and quest-assort unlocks with restart persistence.
- Preserves the `~4.1.0` compatibility range and validates builds and server startup against exact SPT 4.1.5 inputs.
- Supports standalone Trader installation and a validated Trader + Economy Admiral package.
- Ships the MSA ACH TC-2002, TacTec and A18 offers with their complete native armor sections and plates.
- Validates clean installation, replacement upgrade, clean removal and stale removed-trader recovery guidance.

## 0.1.0 — frozen historical baseline

- Established the persistent Admiral identity, 31 quests and 11 finite offers.
- Remains an immutable historical reference; active development continues from 0.2.0.
