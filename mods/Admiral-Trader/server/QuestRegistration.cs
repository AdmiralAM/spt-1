...
public sealed class AdmiralQuestRegistration(ModHelper modHelper, TemplateTable templateTable, LocaleTable localesTable, ISptLogger<AdmiralQuestRegistration> logger) : IOnLoad
{
    ...
    private static readonly string[] RequiredLocaleFields = ["name", "description", "note", "startedMessageText", "successMessageText", "failMessageText", "acceptPlayerMessage", "declinePlayerMessage", "completePlayerMessage", "changeQuestMessageText"];
    ...
    private static void ValidateQuests(Dictionary<MongoId, Quest> quests)
    {
        ...
        foreach (var (questId, quest) in quests)
        {
            ...
            {
                if (isArsenal)
                    throw new InvalidDataException($"Arsenal quest {questId} must not use HandoverItem");
                ValidateNonFirItemProof(questId, finish);
                accessCount++;
                accessHandoverCount++;
                continue;
        ...
        if (accessCount != ExpectedAccessQuestCount || accessHandoverCount != 1 || arsenalReadinessCount != ExpectedArsenalReadinessQuestCount || arsenalCombatCount != ExpectedArsenalCombatQuestCount)
            throw new InvalidDataException($"Admiral quest mix drifted: Access={accessCount}/{ExpectedAccessQuestCount}, AccessHandover={accessHandoverCount}/1, ArsenalReadiness={arsenalReadinessCount}/{ExpectedArsenalReadinessQuestCount}, ArsenalCombat={arsenalCombatCount}/{ExpectedArsenalCombatQuestCount}");
    }
    ...
    {
        List<MongoId> collisions = quests.Keys.Where(templateTable.Quests.ContainsKey).OrderBy(id => id.ToString(), StringComparer.Ordinal).ToList();
        if (collisions.Count != 0)
            throw new InvalidOperationException($"Cannot register Admiral quests: {collisions.Count} quest id collision(s): {string.Join(", ", collisions)}");
    }
    ...
    private Dictionary<string, string> LoadLocaleSet(string modPath, params string[] localeFiles)
    {
        ...
        {
            Dictionary<string, string> source = modHelper.GetJsonDataFromFile<Dictionary<string, string>>(modPath, $"db/locales/{localeFile}");
    ...
    private static void EnsureLocaleCoverage(string localeCode, Dictionary<string, string> locale, Dictionary<MongoId, Quest> quests)
    {
