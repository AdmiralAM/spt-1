...
def success_rewards(slug: str, stage: dict[str, Any], capability: dict[str, Any] | None) -> list[dict[str, Any]]:
    ...
    if xp:
        rewards.append({"value": xp, "id": ident(slug, "reward-xp"), "type": "Experience", "index": index})
        index += 1
    standing = float(stage.get("standing", 0))
    if standing:
        rewards.append({"value": standing, "id": ident(slug, "reward-standing"), "type": "TraderStanding", "target": TRADER_ID, "index": index})
        index += 1
    ...
    if capability and stage.get("sampleAmmoUnits") and capability.get("tpl"):
        units = int(stage["sampleAmmoUnits"])
        item_id = ident(slug, "reward-ammo-item")
        rewards.append({
            "value": units,
            "id": ident(slug, "reward-ammo"),
            "type": "Item",
            "target": item_id,
            "index": index,
...
def authored_index(spec: dict[str, Any]) -> dict[str, Any]:
    stages_by_slug: dict[str, dict[str, Any]] = {}
    family_by_slug: dict[str, str] = {}
    display_by_family: dict[str, str] = {}
    for family in spec.get("families") or []:
        fid = str(family["id"])
        display_by_family[fid] = str(family.get("displayName") or fid)
        for stage in family.get("stages") or []:
            stages_by_slug[str(stage["slug"])] = stage
            family_by_slug[str(stage["slug"])] = fid
    return {"stagesBySlug": stages_by_slug, "familyBySlug": family_by_slug, "displayByFamily": display_by_family}


def build_templates(plan: dict[str, Any], spec: dict[str, Any], capabilities: dict[str, Any], pools: dict[str, Any]) -> dict[str, Any]:
    if any(x.get("targetSptVersion") != "4.1.3" for x in (plan, spec, capabilities, pools)):
        raise ValueError("all weapon-ammo runtime inputs must target SPT 4.1.3")
    authored = authored_index(spec)
    templates: dict[str, dict[str, Any]] = {}
    deferred: list[dict[str, Any]] = []
    for quest in plan.get("quests") or []:
        slug = str(quest["slug"])
        family = str(quest["family"])
        stage = authored["stagesBySlug"][slug]
        pool = pools["families"][family]
        weapon_ids = sorted({str(row["tpl"]) for row in pool.get("weapons") or []})
        capability = capabilities["families"].get(family)
        if family == "special-weapons" and quest["stage"] == "munitions":
            deferred.append({"questId": quest["id"], "slug": slug, "reason": "special sample TPL requires exact SPT 4.1.3 runtime item verification"})
        name = f"Arsenal Protocol: {authored['displayByFamily'][family]} - {str(quest['stage']).title()}"
        qid = str(quest["id"])
        templates[qid] = {
            "QuestName": name,
            "_id": qid,
            ...
            "conditions": {
                "AvailableForFinish": [elimination_condition(slug, int(stage["kills"]), weapon_ids)],
                "AvailableForStart": start_conditions(quest, authored),
                "Started": [], "Success": [], "Fail": [],
            },
            "description": f"{qid} description",
            "failMessageText": f"{qid} failMessageText",
            "name": f"{qid} name",
            "note": f"{qid} note",
            "traderId": TRADER_ID,
            "location": "any",
            ...
            "secretQuest": False,
            "startedMessageText": f"{qid} startedMessageText",
            "successMessageText": f"{qid} successMessageText",
            "acceptPlayerMessage": f"{qid} acceptPlayerMessage",
    ...
    return {"schemaVersion": 1, "targetSptVersion": "4.1.3", "templates": templates, "deferredRuntimeItems": deferred}
...
def main() -> int:
    ...
    payload = build_templates(*[json.loads(p.read_text(encoding="utf-8")) for p in (args.plan, args.spec, args.capabilities, args.pools)])
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    print(json.dumps({"questCount": len(payload["templates"]), "deferredRuntimeItems": len(payload["deferredRuntimeItems"])}, sort_keys=True))
    return 0
