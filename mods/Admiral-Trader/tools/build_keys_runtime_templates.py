...
def start_conditions(quest: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    result: list[dict[str, Any]] = [
        {
            "id": condition_id(slug, "level"),
            "conditionType": "Level",
            "compareMethod": ">=",
            "value": int(quest["minimumLevel"]),
            "dynamicLocale": False,
            "index": 0,
            "parentId": "",
    ...
    for index, predecessor in enumerate(quest.get("prerequisites") or [], start=1):
        result.append(
            {
                "id": condition_id(slug, "prerequisite", index),
                "conditionType": "Quest",
                "dynamicLocale": False,
                "globalQuestCounterId": "",
                "index": index,
...
def success_rewards(quest: dict[str, Any]) -> tuple[list[dict[str, Any]], int]:
    ...
    if xp:
        rewards.append({"id": reward_id(slug, "xp"), "index": index, "type": "Experience", "value": xp})
        index += 1

    standing = float(budget.get("standing", 0))
    if standing:
        rewards.append({
            "id": reward_id(slug, "standing"),
            "index": index,
            "type": "TraderStanding",
            "target": TRADER_ID,
            "value": standing,
...
def build_template(quest: dict[str, Any], key_pool: list[str]) -> tuple[dict[str, Any], int]:
    ...
    template = {
        ...
        "note": f"{qid} note",
        "restartable": False,
        "rewards": {"Fail": [], "Started": [], "Success": rewards},
        "secretQuest": False,
        "side": "Pmc",
...
def build_payload(
    ...
    inventory_by_id = {
        ...
        for row in inventory.get("quests") or []
    ...
    return {
        "schemaVersion": 1,
        "status": "draft-runtime-templates",
        "traderId": TRADER_ID,
        "summary": {
            "templateCount": len(templates),
            "deferredUnlockSlotCount": sum(row["unlockSlots"] for row in deferred_unlocks),
            "minimumKeyPoolSize": min((len(values) for values in key_pools.values()), default=0),
            "maximumKeyPoolSize": max((len(values) for values in key_pools.values()), default=0),
        },
        "templates": templates,
        "sourceKeyPools": key_pools,
        "deferredUnlocks": deferred_unlocks,
    }
