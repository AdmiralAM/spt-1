...
if ($runtimeManifest.schemaVersion -ne 2 -or $runtimeManifest.version -ne '0.3.0' -or $runtimeManifest.sptCompatibility -ne '~4.1.0' -or $runtimeManifest.releaseChannel -ne 'stable-beta') { throw 'Trader stable-beta metadata drift.' }
...
$stagedManifest | Add-Member -NotePropertyName publicationMode -NotePropertyValue 'stable-beta' -Force
...
$provenance = [ordered]@{
    ...
    authority = 'single live Admiral Trader implementation PR exact head'
