...
def validate(spec: dict[str, Any], reward_policy: dict[str, Any], audit: dict[str, Any] | None = None) -> dict[str, int]:
    ...
    for family in families:
        ...
        for index, stage in enumerate(stages):
            ...
            xp = float(stage.get("xp", 0))
            rub = float(stage.get("rub", 0))
            ...
            if index == 2:
                if unlock_slots != 1:
                    fail(f"{slug}: munitions stage must use exactly one controlled unlock slot")
    ...
    if audit is not None:
        ...
        if int(legacy.get("questCount", 0)) != int((spec.get("legacySource") or {}).get("questCount", -1)):
            fail("legacy weapon/ammo quest count drift between audit and authored spec")
        if int(legacy.get("totalAssortmentUnlocks", 0)) != int((spec.get("legacySource") or {}).get("assortmentUnlockCount", -1)):
            fail("legacy weapon/ammo unlock count drift between audit and authored spec")
        if int(legacy.get("crossBundleEdgeCount", 0)) != int((spec.get("legacySource") or {}).get("crossBundleEdgeCount", -1)):
...
def main() -> int:
    ...
    summary = validate(spec, policy, audit)
