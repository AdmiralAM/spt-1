...
STEPS = [
 ("Cold Wake", "Холодный след", 28, "Confirm the route to Boreas and return alive.", "Подтвердить маршрут к «Борею» и вернуться живым."),
 ("Silent Manifest", "Немой манифест", 29, "Recover intact logistics records before the trail goes cold.", "Добыть целые логистические документы, пока след не остыл."),
 ("Outer Watch", "Внешний дозор", 30, "Break the scavenger screen around the stranded ship.", "Разбить заслон мародёров вокруг застрявшего судна."),
 ("Frozen Repair", "Замёрзший ремонт", 31, "Mark the abandoned repair point that supported Boreas.", "Отметить заброшенный ремонтный пункт, обслуживавший «Борей»."),
 ("Resort Relay", "Курортный ретранслятор", 32, "Clear the intercepted relay camp at the resort.", "Зачистить перехваченный узел связи у санатория."),
 ("Broken Transit", "Разорванный транзит", 33, "Mark both ends of the old Boreas transfer route.", "Отметить оба конца старого маршрута снабжения «Борея»."),
 ("Close Quarters", "Ближний борт", 34, "Prove a compact boarding loadout inside the ship perimeter.", "Проверить компактный абордажный комплект в периметре судна."),
 ("Fuel Ledger", "Топливный реестр", 35, "Secure fuel-control evidence and verify the ship remains reachable.", "Добыть сведения о топливе и подтвердить доступность судна."),
 ("Command Deck", "Командная палуба", 37, "Remove armed contractors holding the upper deck.", "Устранить вооружённых контрактников на верхней палубе."),
 ("Boreas Protocol", "Протокол «Борей»", 39, "Complete one decisive sweep and leave with the report.", "Провести решающую зачистку и вынести отчёт."),
]
...
def kills(q, value, target="Savage", weapon=None, zone=None):
 ...
 conditions=[c,location(q,"kills",MAP)]
...
def reward(q, order):
 ...
 return [{"value":xp,"id":hid(q+"xp"),"type":"Experience","index":0},{"value":rep,"id":hid(q+"rep"),"type":"TraderStanding","target":TRADER,"index":1},{"value":rub,"id":hid(q+"rub"),"type":"Item","target":iid,"index":2,"items":[{"_id":iid,"_tpl":ROUBLES,"upd":{"StackObjectsCount":rub}}]}],xp,rub,rep
...
def main():
 ...
 for order,(en_name,ru_name,level,en_brief,ru_brief) in enumerate(STEPS,1):
  ...
  elif order==5: finish=[kills(qid,6,"Any",zone="boreas_camp_resort")] ; en_req=["Eliminate 6 hostiles inside the Boreas resort camp on Shoreline"] ; ru_req=["Устранить 6 противников в лагере «Борея» у санатория на Берегу"]
