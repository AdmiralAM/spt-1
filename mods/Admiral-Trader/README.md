...
Painter/TGC and WTT Artem are optional content providers for the current consolidation candidate. When present, their persistent quests, offers, item trees, 44 quest unlocks and 68 suits remain intact but appear under Admiral; the separate Painter and Artem trader tabs are removed after loaded profiles have been migrated and saved. This adds 35 quests and 402 finite offer roots, producing 207 quests without Icebreaker or 217 with Icebreaker. Existing Admiral content is not replaced. TGC item registration remains owned by Admiral TGC Integration and Belt/filter policy remains owned by B&A&HB. A content-only Painter import supplies its data and five persistent custom templates without loading the retired Painter trader DLL. Removing any provider simply leaves that optional layer absent. See `docs/painter-artem-consolidation-audit.md` for exact source versions, identities, counts and migration behavior.
