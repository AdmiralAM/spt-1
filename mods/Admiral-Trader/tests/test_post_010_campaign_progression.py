...
def test_post_010_campaign_progression_is_complete_reachable_and_acyclic():
    ...
    assert len(phase_ops) == 20 and len(set(phase_ops)) == 20 and set(phase_ops) == expected
    assert set(graph["prerequisites"]) == expected and "expedition-discipline" not in expected
    ...
    critical={"high-value-target-window","labs-security-disruption"}; assert set(graph["phases"][-1]["operations"]) == critical
...
def test_post_010_campaign_has_concrete_non_regressing_player_level_placement():
    ...
    contracts=graph["progressionContracts"]; assert contracts["operationCount"]==20
