...
class Spt415LifecycleContractAuditTests(unittest.TestCase):
    def test_summarizes_native_lifecycle_fields_and_correlations(self):
        quests = {
            "q1": {
        ...
        self.assertEqual(summary["statusValues"], {"1": 1, "2": 1})
        self.assertEqual(summary["sptStatusValues"], {"<missing>": 2})
        self.assertEqual(summary["instantComplete"], {"false": 2})
        self.assertEqual(summary["restartable"], {"false": 1, "true": 1})
        self.assertEqual(summary["statusPresenceCount"], 2)
        self.assertEqual(summary["sptStatusPresenceCount"], 0)
        self.assertEqual(
            summary["lifecycleFieldCorrelations"],
            {
                "accept=eft|progress=<missing>|status=2": 1,
                "accept=eft|progress=eft|status=1": 1,
            },
        )
        self.assertEqual(
            summary["lifecycleFieldCorrelationExamples"]["accept=eft|progress=eft|status=1"],
            ["q1"],
        )
