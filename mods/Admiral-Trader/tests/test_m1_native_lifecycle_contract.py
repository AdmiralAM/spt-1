...
class M1NativeLifecycleContractTests(unittest.TestCase):
    ...
    def test_all_31_templates_require_explicit_native_accept_and_complete(self):
        ...
        for path, quest in self.quests:
            with self.subTest(path=path.name):
                ...
                self.assertEqual(quest.get("status"), 0)
                self.assertNotIn("sptStatus", quest)
                self.assertIs(quest.get("restartable"), False)
                self.assertEqual(quest.get("progressSource"), "eft")
                self.assertEqual(quest.get("gameModes"), [])
                self.assertEqual(quest.get("rankingModes"), [])
                self.assertEqual(quest.get("arenaLocations"), [])

    def test_templates_do_not_encode_automatic_lifecycle_state_conditions(self):
        for path, quest in self.quests:
            conditions = quest.get("conditions") or {}
            with self.subTest(path=path.name):
                self.assertNotIn("Started", conditions)
                self.assertNotIn("Success", conditions)
    ...
    def test_clean_level_one_profile_has_one_manual_offer(self):
        ...
        for _, quest in self.quests:
            ...
            if any(condition["conditionType"] == "Quest" for condition in start):
                continue
