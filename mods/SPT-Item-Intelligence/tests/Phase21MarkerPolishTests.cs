...
static class Phase21MarkerPolishTests
{
    public static int Run()
    {
        ...
        Expect(settings.Contains("enum ItemMarkerSymbol") && settings.Contains("Check") && settings.Contains("Cross") && settings.Contains("Diamond") &&
               settings.Contains("Dot") && settings.Contains("Plus") && settings.Contains("Chevron") && settings.Contains("Bolt") &&
               settings.Contains("Shield") && settings.Contains("Target") && settings.Contains("Spark") &&
               settings.Contains("\"Symbol\", ItemMarkerSymbol.Check"),
            "marker offers ten original small-size symbol choices", ref assertions);
        ...
        Expect(overlay.Contains("static Sprite SampledSprite") && overlay.Contains("const int samples = 2") &&
               overlay.Contains("Four samples per output pixel"),
            "tiny procedural symbols use deterministic supersampled antialiasing", ref assertions);
