...
namespace SPTItemIntelligence
{
    public sealed class ItemIntelligenceUiSettings
    {
        ...
        {
            tooltipMode = config.Bind("Tooltip", "Mode", ItemTooltipMode.Normal, "Minimal: Value + Keep. Normal: adds Quest Now/Later and Hideout. Detailed: adds best source, per-slot, owned and up to three concrete targets. Full: adds all targets, decision and template id.");
            ...
            markerOffsetX = config.Bind("Marker", "Offset X", 3f, new ConfigDescription("Horizontal inset from the item card top-right corner; negative values move outward.", new AcceptableValueRange<float>(-40f, 40f)));
