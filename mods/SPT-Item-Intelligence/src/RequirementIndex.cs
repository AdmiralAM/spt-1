...
{
    public enum RequirementSource { CurrentQuest, FutureQuest, Hideout }
    public enum RequirementCombineMode { Additive, AlternativeMaximum }
    ...
    [Flags]
    public enum RequirementReasonFlags { None = 0, CurrentQuest = 1, FutureQuest = 2, Hideout = 4, FoundInRaid = 8 }
    ...
    public sealed class RequirementProjection
    {
        ...
        public RequirementProjection(long generatedAtUnixSeconds, IEnumerable<OwnedTemplateCount> owned, IEnumerable<RequirementContribution> contributions)
        {
            GeneratedAtUnixSeconds = Math.Max(0, generatedAtUnixSeconds); this.owned = Copy(owned).AsReadOnly(); this.contributions = Copy(contributions).AsReadOnly();
        }
        ...
        static List<T> Copy<T>(IEnumerable<T> source) where T : class { List<T> result = new List<T>(); if (source != null) foreach (T value in source) if (value != null) result.Add(value); return result; }
    }
    ...
    public sealed class RequirementIndex
    {
        ...
        internal RequirementIndex(long generatedAtUnixSeconds, IDictionary<string, RequirementIndexEntry> entries) { GeneratedAtUnixSeconds = Math.Max(0, generatedAtUnixSeconds); this.entries = new ReadOnlyDictionary<string, RequirementIndexEntry>(new Dictionary<string, RequirementIndexEntry>(entries, StringComparer.Ordinal)); }
        public static RequirementIndex Empty => empty;
        public long GeneratedAtUnixSeconds { get; }
        public int Count => entries.Count;
        public IReadOnlyDictionary<string, RequirementIndexEntry> Entries => entries;
        public RequirementIndexEntry Get(string templateId) => GetNormalized(RequirementContribution.NormalizeId(templateId));
        public RequirementIndexEntry GetNormalized(string normalizedTemplateId) { if (string.IsNullOrEmpty(normalizedTemplateId)) return RequirementIndexEntry.Empty; RequirementIndexEntry entry; return entries.TryGetValue(normalizedTemplateId, out entry) ? entry : RequirementIndexEntry.Empty; }
    }
    ...
    public sealed class RequirementIndexStore
    {
        ...
        public bool TryRefresh(RequirementDataEnvelope snapshot, IRequirementDataProjector projector, RequirementIndexOptions options, out string error)
        {
            if (snapshot == null) { error = "Snapshot is missing."; return false; }
            if (!snapshot.profileReady) { error = "Profile is not ready."; return false; }
            if (projector == null) { error = "Requirement projector is missing."; return false; }
            try
            {
                RequirementProjection projection = projector.Project(snapshot);
                if (projection == null) { error = "Requirement projection is missing."; return false; }
                RequirementIndex replacement = RequirementIndexBuilder.Build(projection, options); Interlocked.Exchange(ref current, replacement); error = null; return true;
            }
