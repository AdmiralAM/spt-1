...
| Ammo penetration class badge | Shows EFT's native I–VI armor-class sprite on ammo ItemViews at the highest class with at least 20% native EFT penetration chance at 100% armor durability, with a class-I fallback for a lower nonzero chance. Muted sprite tint conveys the chance tier on that class. | Adds an optional penetration cue without changing ammo background colors or the existing information marker. | No ammo-table catalog, external ammo-mod dependency or ballistic rule replacement. |
...
| Amands Sense adapter | Optional soft adapter for requirement-first loot, key/grenade/currency/food/water categories, stock/count colors on Sense's native container counter, category-hued marker brightness based on total container value, pickup/drop/reset/rescan events and a smaller native container-name line. Reuses Sense-loaded sprites. | Unmet quest/hideout requirements stay first; a category in the container outranks a completed requirement; drinks use item type/hydration rather than name matches, and native rare/favorite/value marks stay protected. Container value changes marker brightness only and never adds a price label. | Complements Amands Sense and leaves its body, exfil, sound and base world rendering intact. Item Intelligence ships no Sense assets or config rewrite. |
...
| `AmmoPenetrationBadgeView.cs` | Compact native EFT I–VI class sprite, anchored clear of stack counts; five subdued chance tints. |
...
- `Amands Sense / Integration`, `Amands Sense / Required Items`, `Amands Sense / Category Markers`, `Amands Sense / Container Value Colors`, `Amands Sense / Secondary Reason Outline`, `Amands Sense / Remaining Count Text`, `Amands Sense / Text Scale`, `Amands Sense / Container Name Scale`.
