...
| Ammo penetration class badge | Shows a compact I–VI shield on ammo ItemViews at the highest class with at least 20% native EFT penetration chance at 100% armor durability, with a class-I fallback for a lower nonzero chance. Muted shield color conveys the chance tier on that class. | Adds an optional penetration cue without changing ammo background colors or the existing information marker. | No ammo-table catalog, external ammo-mod dependency or ballistic rule replacement. |
...
| `AmmoPenetrationBadgeView.cs` | Compact I–VI UI text over a shield silhouette, anchored clear of stack counts; five subdued chance colors. |
