...
internal static class AdmiralTraderAdapterSmoke
{
    [ModuleInitializer]
    internal static void Run()
    {
        ...
        MustFail("invalid stock cap", () => AdmiralTraderAdapterEvidence.ParseGameplayPolicy(maintainedPolicy.Replace("\"maximumPermanentOfferStockPerReset\": 80", "\"maximumPermanentOfferStockPerReset\": 0")));
    ...
    {
        if (!condition) throw new InvalidOperationException($"Economy Admiral Admiral Trader adapter smoke: {message}");
    }
    ...
    private static void MustFail(string name, Action action)
    {
        ...
        throw new InvalidOperationException($"Economy Admiral Admiral Trader adapter smoke expected '{name}' to fail.");
    }
}
