...
/// </summary>
[Injectable]
public sealed class DirectPrimaryAuditService(
    TemplateTable templates,
    TradersTable traders,
    ModHelper modHelper,
    EconomyRuntimeConfigService runtimeConfigService,
    ISptLogger<DirectPrimaryAuditService> logger
)
{
    ...
    public async Task RunAsync(VanillaBaselineSnapshot baseline, CancellationToken cancellationToken)
    {
        ...
        var root = JsonSerializer.SerializeToNode(report, JsonOptions)?.AsObject()
            ?? throw new InvalidOperationException("Economy Admiral direct primary audit serialization failed.");
        root["VanillaBenchmarkSource"] = "PristineStartupSnapshot";
        root["PristineQuestCount"] = baseline.QuestCount;

        var modPath = modHelper.GetAbsolutePathToModFolder(typeof(DirectPrimaryAuditService).Assembly);
        ...
        logger.Info(
            $"[Economy Admiral] direct primary audit complete from typed final DB + pristine startup snapshot: " +
