...
internal static class ReloadEpochTerminalCompatibilityRegression
{
    ...
    internal static void Run()
    {
        ...
        Require(source, "terminalFailure = true;\n                        return false;",
            "loaded-but-structurally-incompatible Harmony/runtime contracts must become terminal fail-closed states");
        ...
        if (harmonyPresent < 0 || structuralGate < 0 || terminalAssignment < 0 || ownerCreate < 0
            || !(harmonyPresent < structuralGate && structuralGate < terminalAssignment && terminalAssignment < ownerCreate))
            throw new InvalidOperationException("Reload epoch terminal compatibility regression failed: structural incompatibility must terminate retries before any Harmony owner/process-wide mutation is created.");
    }
