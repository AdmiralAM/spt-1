...
| --- | --- | --- | --- | --- |
| 1 | Start SPT, open the **Items** inventory once, then proceed to a PMC raid's pre-raid **Insurance** screen. Do not switch tabs to force a refresh. | Existing vanilla equipment remains intact; Belt is in dedicated slot 15; HeadBand is in slot 16 at the intended Headwear reflow position; the insurer has a real localized name; Back/Next/Insure remain responsive; client log has `INSURANCE-SAFE SLOT PROOF`; server log has a successful protection update and no `/b-and-hb/protection` JSON error. | `DEFAULT_VALUE` insurer, frozen navigation, `Collection was modified` from `EquipmentTab.Show`, `/b-and-hb/protection` request failure, missing/duplicated vanilla equipment, or startup/load failure. | One screenshot + client/server lines around the first insurance entry only on FAIL. |
