...
public sealed class RuntimeCandidateAssort(TradersTable tradersTable, ISptLogger<RuntimeCandidateAssort> logger) : IOnLoad
{
    ...
    {
        var trader = tradersTable.GetValueOrDefault(RuntimeCandidateOfferContract.RagmanTraderId) ?? throw new InvalidOperationException("B&A&HB RC could not find Ragman.");
        ...
        if (existing != null)
        {
            ...
            logger.Success($"B&A&HB RC retained existing validated Ragman LL{RuntimeCandidateOfferContract.LoyaltyLevel} offer for {RuntimeCandidateOfferContract.PriceRoubles:N0} RUB.");
            return Task.CompletedTask;
        }
        trader.Assort.Items.Add(new Item { Id = id, Template = new MongoId(RuntimeCandidateBeltItem.RuntimeCandidateTpl), ParentId = RuntimeCandidateOfferContract.RootId, SlotId = RuntimeCandidateOfferContract.RootId, Upd = new Upd { UnlimitedCount = true, StackObjectsCount = RuntimeCandidateOfferContract.UnlimitedStock } });
        ...
        logger.Success($"B&A&HB RC added to Ragman LL{RuntimeCandidateOfferContract.LoyaltyLevel} for {RuntimeCandidateOfferContract.PriceRoubles:N0} RUB.");
        return Task.CompletedTask;
        ...
        void ValidateExistingAssort()
        {
            ...
            if (!trader.Assort.BarterScheme.TryGetValue(id, out var schemes)
                ...
                throw new InvalidOperationException($"B&A&HB RC assort ID collision: existing offer does not cost exactly {RuntimeCandidateOfferContract.PriceRoubles:N0} RUB.");

            if (!trader.Assort.LoyalLevelItems.TryGetValue(id, out var loyaltyLevel) || loyaltyLevel != RuntimeCandidateOfferContract.LoyaltyLevel)
                throw new InvalidOperationException($"B&A&HB RC assort ID collision: existing offer is not registered at Ragman LL{RuntimeCandidateOfferContract.LoyaltyLevel}.");
        }
    }
}
