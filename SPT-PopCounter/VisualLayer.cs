...
{
    [BepInPlugin("com.admiralam.spt.tacticalhud.visuals", "SPT Tactical HUD Visual Layer", "1.10.1")]
    ...
    public sealed class VisualLayer : BaseUnityPlugin
    {
        ...
        float Text(Rect root,string value,float x,float y,int size,float opacity,Color color,float alphaScale=1f)
        {
            ...
            Color main=color;main.a*=effective;
            float w=text.CalcSize(new GUIContent(value)).x;
            Rect r=new Rect(root.x+x,root.y+y,w+4,size+7);
            Color old=text.normal.textColor;

            text.normal.textColor=new Color(0,0,0,Mathf.Clamp01(effective*.92f));
            ...
            text.normal.textColor=new Color(0,0,0,Mathf.Clamp01(effective*.42f));
        ...
        float Icon(Rect root,string key,float x,float y,int size,float opacity,Color color,float scale=1f)
        {
            ...
            Rect rr=new Rect(root.x+x,root.y+y-2,px,px);
            Color old=GUI.color;
            GUI.color=new Color(0,0,0,Mathf.Clamp01(opacity*.72f));
            GUI.DrawTexture(new Rect(rr.x+1,rr.y+1,rr.width,rr.height),t,ScaleMode.ScaleToFit,true);
            Color c=color;c.a*=opacity;
            GUI.color=c;
            GUI.DrawTexture(rr,t,ScaleMode.ScaleToFit,true);
            GUI.color=old;
            return x+px+2;
        }

        static float Gap(float x,float amount){return x+amount;}
        ...
        void EditSurface(int id,Rect r,ConfigEntry<float>xEntry,ConfigEntry<float>yEntry,bool fromBottom)
        {
            ...
            Color old=GUI.color;GUI.color=new Color(1f,1f,1f,.05f);GUI.Box(r,string.Empty);GUI.color=old;
        ...
        {
            int size=Cfg("popSize",10);
            float op=Cfg("popOpacity",.55f),xPos=Cfg("popX",8f),bottom=Cfg("popY",8f);
            Rect r=new Rect(xPos,Screen.height-bottom-(size+9),176,size+9);
            EditSurface(1,r,Entry<float>("popX"),Entry<float>("popY"),true);
            float x=0;
            ...
            x=Icon(r,"usec",x,0,size,op,pmc,.90f);x=Text(r,Value("pmc",0).ToString(),x,0,size,op,Neutral);x=Gap(x,7);
            x=Icon(r,"scav",x,0,size,op,scav,.90f);x=Text(r,Value("scav",0).ToString(),x,0,size,op,Neutral);x=Gap(x,7);
            x=Icon(r,"boss",x,0,size,op,boss,.90f);x=Text(r,Value("boss",0).ToString(),x,0,size,op,Neutral);x=Gap(x,7);
            x=Icon(r,"raider",x,0,size,op,raid,.90f);Text(r,Value("reinforced",0).ToString(),x,0,size,op,Neutral);
        }

        void DrawStatus(bool editing)
        {
            int size=Cfg("statusSize",10);
            float op=Cfg("statusOpacity",.55f),xPos=Cfg("statusX",8f),bottom=Cfg("statusY",24f);
            Rect r=new Rect(xPos,Screen.height-bottom-(size+9),188,size+9);
            EditSurface(2,r,Entry<float>("statusX"),Entry<float>("statusY"),true);
            float x=0;
            ...
            x=Icon(r,"water",x,0,size,op,Water,.90f);x=Text(r,Mathf.RoundToInt(hydration).ToString(),x,0,size,op,Neutral);x=Gap(x,8);
            x=Icon(r,"energy",x,0,size,op,Energy,.90f);x=Text(r,Mathf.RoundToInt(energy).ToString(),x,0,size,op,Neutral);x=Gap(x,8);

            Color wc=Cfg("weightOk",new Color(.58f,.75f,.52f));
            int severity=1;
            if(over>0&&weight>=over){wc=Cfg("weightHeavy",new Color(.78f,.68f,.39f));severity=2;}
            if(walk>0&&weight>=walk){wc=Cfg("weightCritical",new Color(.75f,.42f,.39f));severity=3;}
            x=Icon(r,"weight",x,0,size,op,Muted,.86f);
            x=Text(r,Mathf.RoundToInt(weight).ToString(),x,0,size,op,Neutral);
            x=Text(r,"kg",x,1,Mathf.Max(8,size-2),op,Muted,.82f);
            x=Gap(x,3);
            Icon(r,"weight"+severity,x,0,size,op,wc,.70f);
        }

        void DrawKillFeed(bool editing)
        {
            int size=Cfg("killSize",10),max=Cfg("killMax",3);
            float op=Cfg("killOpacity",.55f),xPos=Cfg("killX",1500f),yPos=Cfg("killY",100f),life=Cfg("killLifetime",6f);
            string mode=Cfg("killMode","Normal");
            object listObj=F("kills")?.GetValue(runtime);
            IEnumerable list=listObj as IEnumerable;
            var entries=new List<object>();if(list!=null)foreach(object k in list)entries.Add(k);
            int rows=editing?Mathf.Max(1,Mathf.Min(max,entries.Count==0?1:entries.Count)):Mathf.Max(1,Mathf.Min(max,entries.Count));
            float width=mode=="Detailed"?320f:(mode=="Minimal"?150f:240f);
            Rect r=new Rect(xPos,yPos,width,(size+8)*rows);
            EditSurface(3,r,Entry<float>("killX"),Entry<float>("killY"),false);
            ...
            {
                object k=entries[i];
                float created=ToFloat(ReadMember(k,"Created")),age=Time.unscaledTime-created;
                float fade=Mathf.Clamp01((life-age)/fadeWindow);
                DrawKillRow(r,ReadMember(k,"Killer")?.ToString(),ReadMember(k,"Victim")?.ToString(),CleanWeapon(ReadMember(k,"Weapon")?.ToString()),ReadMember(k,"Hit")?.ToString(),ToFloat(ReadMember(k,"Distance")),ToBool(ReadMember(k,"HasDistance")),shown,fade,mode,size,op);
            }
        }
        ...
        {
            float y=row*(size+8),x=0,op=opacity*fade;
            Color kc=RoleColor(killer),vc=RoleColor(victim);

            x=Icon(r,RoleIcon(killer),x,y,size,op,kc,.86f);
            x=Text(r,ShortRole(killer),x,y,size,op,kc,.98f);
            x=Gap(x,7);

            if(mode!="Minimal")
            {
                x=Icon(r,WeaponKey(weapon),x,y,size,op,Neutral,.96f);
                if(mode=="Detailed"&&weapon!="?")
                {
            ...
            x=Icon(r,RoleIcon(victim),x,y,size,op,vc,.86f);
            x=Text(r,ShortRole(victim),x,y,size,op,vc,.98f);

            if(mode!="Minimal")
            {
                x=Gap(x,6);
                string hk=HitKey(hit);
                x=Icon(r,hk,x,y,size,op,hk=="head"?Head:Muted,.76f);
                if(hasDistance)
                {
                    x=Gap(x,1);
        ...
        static string CleanWeapon(string raw)
        {
            ...
            string s=raw.Trim();int b=s.IndexOf('[');if(b>=0)s=s.Substring(0,b).Trim();
            s=s.Replace("ShortName","").Replace("Template","").Trim(' ','[',']','(',')','{','}');if(s.Length==0)return"?";
            string compact=s.Replace("-","").Replace("_","").Replace(" ","");bool hexLike=compact.Length>=20;
            for(int i=0;i<compact.Length&&hexLike;i++)if(!Uri.IsHexDigit(compact[i]))hexLike=false;
            if(hexLike)return"?";if(s.Length>22)s=s.Substring(0,22).Trim();return s;
        }
