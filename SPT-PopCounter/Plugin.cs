...
{
    [BepInPlugin("com.admiralam.spt.tacticalhud", "SPT Tactical HUD", "1.7.0")]
    public sealed class Plugin : BaseUnityPlugin
    {
        ...
        ConfigEntry<Color> pmcColor, scavColor, bossColor, reinforcedColor, statusColor, weightOk, weightHeavy, weightCritical;
        ...
        class Tracked { public object Player; public bool Alive; public Vector3 Pos; public string Kind; public string Weapon; }
        class KillLine { public string Killer, Victim, Weapon, Hit; public float Distance, Created; public bool HasDistance; }
        class VersionTarget { public object Owner; public MemberInfo Member; public string Original; }
        ...
        void Awake()
        {
            ...
            statusOutside = Config.Bind("Player Status", "Show Outside Raid", false, "Show outside raid if data exists");
            ...
            statusColor = C("Player Status Colors", "Hydration Energy", .72f, .76f, .76f);
            ...
            killLifetime = Config.Bind("Kill Feed", "Lifetime", 6f, new ConfigDescription("Seconds before fade", new AcceptableValueRange<float>(2, 15)));
            killMax = Config.Bind("Kill Feed", "Max Entries", 3, new ConfigDescription("Visible lines", new AcceptableValueRange<int>(1, 6)));

            Logger.LogInfo("SPT Tactical HUD v1.7.0 loaded");
        }
        ...
        void Update()
        {
            ...
            if (Time.unscaledTime >= nextVersionScan) { nextVersionScan = Time.unscaledTime + 1f; ApplyVersion(); }
        ...
        Rect PopRect() => new Rect(popX.Value, Screen.height - popY.Value - (popSize.Value + 8), Mathf.Max(112, popSize.Value * 14), popSize.Value + 8);
        Rect StatusRect() => new Rect(statusX.Value, Screen.height - statusY.Value - (statusSize.Value + 8), Mathf.Max(150, statusSize.Value * 18), statusSize.Value + 8);
        Rect KillRect() => new Rect(killX.Value, killY.Value, killMode.Value == "Minimal" ? 190 : 270, (killSize.Value + 8) * Mathf.Max(1, killMax.Value));
        ...
        float Icon(Rect r, string key, float x, float y, int sz, float op, Color c)
        {
            float px = Mathf.Max(10, sz + 6); c.a *= op; Color old = GUI.color; GUI.color = c;
            GUI.DrawTexture(new Rect(r.x + x, r.y + y - 1, px, px), icons.Get(key), ScaleMode.ScaleToFit, true);
            GUI.color = old; return x + px + 2;
        }
        ...
        void DrawPop(Rect r)
        {
            ...
            x = Icon(r, "usec", x, 0, popSize.Value, popOpacity.Value, pmcColor.Value); x = Label(r, pmc.ToString(), x, 0, popSize.Value, popOpacity.Value, pmcColor.Value);
            x = Icon(r, "scav", x, 0, popSize.Value, popOpacity.Value, scavColor.Value); x = Label(r, scav.ToString(), x, 0, popSize.Value, popOpacity.Value, scavColor.Value);
            x = Icon(r, "boss", x, 0, popSize.Value, popOpacity.Value, bossColor.Value); x = Label(r, boss.ToString(), x, 0, popSize.Value, popOpacity.Value, bossColor.Value);
            x = Icon(r, "raider", x, 0, popSize.Value, popOpacity.Value, reinforcedColor.Value); Label(r, reinforced.ToString(), x, 0, popSize.Value, popOpacity.Value, reinforcedColor.Value);
        }

        void DrawStatus(Rect r)
        {
            BoxDrag(2, r); float x = 0;
            x = Icon(r, "water", x, 0, statusSize.Value, statusOpacity.Value, new Color(.48f, .70f, .88f)); x = Label(r, Mathf.RoundToInt(hydration).ToString(), x, 0, statusSize.Value, statusOpacity.Value, statusColor.Value);
            x = Icon(r, "energy", x, 0, statusSize.Value, statusOpacity.Value, new Color(.90f, .72f, .28f)); x = Label(r, Mathf.RoundToInt(energy).ToString(), x, 0, statusSize.Value, statusOpacity.Value, statusColor.Value);
            Color wc; string loadIcon;
            if (maxWeight <= 0 || weight / maxWeight < .75f) { wc = weightOk.Value; loadIcon = "weight1"; }
            else if (weight / maxWeight < .95f) { wc = weightHeavy.Value; loadIcon = "weight2"; }
            else { wc = weightCritical.Value; loadIcon = "weight3"; }
            x = Icon(r, "weight", x, 0, statusSize.Value, statusOpacity.Value, new Color(.72f, .74f, .74f));
            x = Label(r, Mathf.RoundToInt(weight) + "/" + Mathf.RoundToInt(maxWeight), x, 0, statusSize.Value, statusOpacity.Value, wc);
            Icon(r, loadIcon, x, 0, statusSize.Value, statusOpacity.Value, wc);
        }
        ...
        void DrawKills(Rect r, bool editing)
        {
            ...
            if (kills.Count == 0)
            {
                if (editing)
                {
                    var p = new KillLine { Killer = "USEC", Victim = "Scav", Weapon = "AK-74", Hit = "Head", Distance = 187, HasDistance = true, Created = Time.unscaledTime };
                    DrawKillRow(r, p, 0, 1f);
                }
                return;
            }
        ...
        {
            float y = row * (killSize.Value + 8), x = 0, op = killOpacity.Value * fade;
            Color kc = RoleColor(k.Killer), vc = RoleColor(k.Victim);
            x = Icon(r, RoleIcon(k.Killer), x, y, killSize.Value, op, kc); x = Label(r, k.Killer, x, y, killSize.Value, op, kc);
            x = Icon(r, WeaponIconKey(k.Weapon), x + 3, y, killSize.Value, op, FixedNeutral);
            x = Icon(r, RoleIcon(k.Victim), x + 3, y, killSize.Value, op, vc); x = Label(r, k.Victim, x, y, killSize.Value, op, vc);
            if (killMode.Value != "Minimal")
            {
                string hk = HitIconKey(k.Hit); Color hc = hk == "head" ? FixedHeadshot : FixedNeutral;
                x = Icon(r, hk, x + 3, y, killSize.Value, op, hc);
                if (k.HasDistance) Label(r, Mathf.RoundToInt(k.Distance) + "m", x + 2, y, killSize.Value, op, FixedNeutral);
            }
        }

        Color RoleColor(string kind)
        {
            if (kind == "USEC" || kind == "BEAR" || kind == "PMC") return FixedPmc;
            if (kind == "Scav") return FixedScav;
            if (kind == "Boss") return FixedBoss;
            if (kind == "Raider") return FixedRaider;
            return FixedNeutral;
        }
        string RoleIcon(string kind)
        {
            if (kind == "USEC") return "usec";
            if (kind == "BEAR") return "bear";
            if (kind == "Scav") return "scav";
            if (kind == "Boss") return "boss";
            if (kind == "Raider") return "raider";
            return "usec";
        }
        string HitIconKey(string h)
        {
            h = (h ?? "").ToLowerInvariant();
            if (h.Contains("head")) return "head";
            if (h.Contains("arm")) return "arm";
            if (h.Contains("leg")) return "leg";
            if (h.Contains("stomach")) return "stomach";
            return "torso";
        }
        string WeaponIconKey(string w)
        {
        ...
        void Refresh()
        {
            try
            {
                object w = GetWorld(); if (w == null) { Outside(); return; }
                IEnumerable ps = GetPlayers(w); if (ps == null) { Outside(); return; }
                inRaid = true; int p = 0, s = 0, b = 0, r = 0; object local = null; var seen = new HashSet<string>();
                foreach (object pl in ps)
                {
                    if (pl == null) continue; string id = PlayerId(pl); if (string.IsNullOrEmpty(id)) id = pl.GetHashCode().ToString(); seen.Add(id);
                    bool alive = IsAlive(pl); string kind = Kind(pl); Vector3 pos = Position(pl);
                    if (IsTrue(ReadMember(pl, "IsYourPlayer"))) local = pl;
                    else if (alive) { if (kind == "Boss") b++; else if (kind == "Raider") r++; else if (kind == "USEC" || kind == "BEAR" || kind == "PMC") p++; else s++; }
                    Tracked old; if (tracked.TryGetValue(id, out old) && old.Alive && !alive) CaptureDeath(pl, old);
                    tracked[id] = new Tracked { Player = pl, Alive = alive, Pos = pos, Kind = kind, Weapon = Weapon(pl) };
                }
                foreach (string id in tracked.Keys.Where(x => !seen.Contains(x)).ToList()) tracked.Remove(id);
                pmc = p; scav = s; boss = b; reinforced = r; RefreshStatus(local);
            }
            catch (Exception ex) { Logger.LogWarning("HUD refresh: " + ex.Message); }
        }

        void CaptureDeath(object victim, Tracked old)
        {
            object hc = ReadMember(victim, "HealthController");
            object info = FirstNonNull(ReadMember(hc, "LastDamageInfo"), ReadMember(victim, "LastDamageInfo"), ReadMember(hc, "DamageInfo"), ReadMember(victim, "DamageInfo"));
            object attacker = ExtractAttacker(info); string hit = ExtractHit(info); string killer = "Unknown", weapon = "?"; float dist = 0; bool hasDist = false;
            if (attacker != null && !ReferenceEquals(attacker, victim))
            {
                killer = Kind(attacker); weapon = Weapon(attacker); Vector3 ap = Position(attacker);
                if (ap != Vector3.zero && old.Pos != Vector3.zero) { dist = Vector3.Distance(ap, old.Pos); hasDist = true; }
            }
            else DiagnoseDeath(victim, hc, info);
            kills.Add(new KillLine { Killer = killer, Victim = old.Kind, Weapon = weapon, Hit = hit, Distance = dist, HasDistance = hasDist, Created = Time.unscaledTime });
        }

        object ExtractAttacker(object info)
        {
            if (info == null) return null;
            foreach (string n in new[] { "Player", "Attacker", "SourcePlayer", "Aggressor", "Killer", "Instigator", "Owner", "Source" }) { object p = NormalizePlayer(ReadMember(info, n)); if (p != null) return p; }
        ...
        {
            if (!killDiagnostics.Value) return; string key = (info?.GetType().FullName ?? "null") + "|" + (hc?.GetType().FullName ?? "null"); if (!diagSeen.Add(key)) return;
            Logger.LogWarning("KillFeed attribution unresolved. victim=" + victim.GetType().FullName + " health=" + (hc?.GetType().FullName ?? "null") + " damage=" + (info?.GetType().FullName ?? "null"));
            if (info != null) Logger.LogInfo("KillFeed DamageInfo members: " + MemberNames(info.GetType(), 32));
            if (hc != null) Logger.LogInfo("KillFeed HealthController candidate members: " + MemberNames(hc.GetType(), 32, "damage", "killer", "attacker", "death", "hit"));
        }
        string MemberNames(Type t, int max, params string[] filters)
        {
            var names = new List<string>(); try { foreach (var m in t.GetMembers(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)) { if (filters != null && filters.Length > 0 && !filters.Any(f => m.Name.IndexOf(f, StringComparison.OrdinalIgnoreCase) >= 0)) continue; if (!names.Contains(m.Name)) names.Add(m.Name); if (names.Count >= max) break; } } catch { }
            return string.Join(",", names.ToArray());
        }
        object FirstNonNull(params object[] values) { foreach (object v in values) if (v != null) return v; return null; }
        ...
        string PlayerId(object p) { object pr = ReadMember(p, "Profile"); return (ReadMember(pr, "Id") ?? ReadMember(pr, "ProfileId") ?? ReadMember(p, "ProfileId"))?.ToString(); }

        void RefreshStatus(object pl)
        {
            if (pl == null) return; object hc = ReadMember(pl, "HealthController");
            ...
            object ph = ReadMember(pl, "Physical"); weight = ReadFloat(ReadMember(ph, "CurrentWeight")) ?? ReadFloat(ReadMember(pl, "Weight")) ?? weight; maxWeight = ReadFloat(ReadMember(ph, "MaxWeight")) ?? ReadFloat(ReadMember(ph, "BaseWeightLimit")) ?? maxWeight;
        }

        void Outside()
        {
            inRaid = false; pmc = scav = boss = reinforced = 0; tracked.Clear(); kills.Clear(); diagSeen.Clear();
        ...
        void ApplyVersion()
        {
            try
            {
                DiscoverVersionTargets();
                foreach (VersionTarget t in versionTargets.ToArray())
                {
                    if (t.Owner == null || t.Member == null) continue;
                    string desired = showVersion.Value ? t.Original : string.Empty;
                    WriteStringMember(t.Owner, t.Member, desired);
                }
            }
            catch (Exception ex) { Logger.LogDebug("Version label control: " + ex.Message); }
        }
        ...
        void DiscoverVersionTargets()
        {
            foreach (var mb in Resources.FindObjectsOfTypeAll<MonoBehaviour>())
            {
                ...
                {
                    if (versionTargets.Any(v => ReferenceEquals(v.Owner, mb) && v.Member.Name == m.Name)) continue;
                    ...
                    Logger.LogInfo("SPT version label target: " + t.FullName + "." + m.Name);
                }
            }
        }
        IEnumerable<MemberInfo> StringMembers(Type t)
        {
            foreach (var p in t.GetProperties(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)) if (p.PropertyType == typeof(string) && p.CanRead && p.CanWrite) yield return p;
            foreach (var f in t.GetFields(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)) if (f.FieldType == typeof(string)) yield return f;
        }
        string ReadStringMember(object o, MemberInfo m)
        {
            try { var p = m as PropertyInfo; if (p != null) return p.GetValue(o, null) as string; var f = m as FieldInfo; if (f != null) return f.GetValue(o) as string; } catch { }
            return null;
        }
        void WriteStringMember(object o, MemberInfo m, string value)
        {
            try { var p = m as PropertyInfo; if (p != null && p.CanWrite) { p.SetValue(o, value, null); return; } var f = m as FieldInfo; if (f != null) f.SetValue(o, value); } catch { }
        }
        ...
        static object ReadMember(object o, string n)
        {
            ...
            try { var p = t.GetProperty(n, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic); if (p != null) return p.GetValue(o, null); } catch { }
            try { var f = t.GetField(n, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic); if (f != null) return f.GetValue(o); } catch { }
            return null;
        }
